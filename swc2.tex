...
\begin{abstract}\begin{normalsize}
  ...
  In modern genetics, \ecoli\ is often used as an expression system to commercially
  produce useful proteins.  However, sequence-dependent
  features, such as rare codons and codon bias, have large effects on translational
  efficiency.  To tackle this problem, we proposed a stochastic model designed to computationally estimate
  translational efficiency and predict frameshifting, uniting ideas from biological literature.  We proposed
  two metrics that bear considerable predictive power for translational efficiency.
  We then tested our model on a large sample of sequences from the \ecoli\ genome and
  found that over 90\% of them are predicted to translate at high yields, a
  result that concurs with biological knowledge.  Moreover, ribosomal proteins
  are predicted to translate at even more efficient rates, as suggested by microbiologists.
  Computational results also concurred with experimental data conducted on translation
  ...
  interactions are discussed.  An experiment to test our model's predictions is
  outlined, although the work is still in progress.  With more data, this model provides great
  promise to serve as a time-effective, low-cost means of predicting translational
  efficiency of recombinant proteins in \ecoli.
