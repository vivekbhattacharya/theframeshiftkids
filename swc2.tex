...
We propose a notion called {\emph{displacement deviation}}, defined by
\begin{equation}
...
using the stochastic model, and most of our results supported those found by 
Weiss.  However, results did not match Weiss's numbers through ordinality.
Rather, we could separate the genes into two general categories:
``high yield" ($\beta$-galactosidase activity of over 1650 
whole-cell units, where unaltered \prfB\ has an activity of 6600) and ``low yield" (otherwise).
With some reservations, the data supports our prediction that high yield
and low yield sequences could more-or-less be distinguished via error-free rate.

Weiss conducted one set of experiments in which he changed the codon before
the frameshift site.  Interestingly, changing this codon from the original {\sc{guu}}
drops the yield down to less than 60 whole-cell units.  However, our model
predicted very high error-free rates, at an average of 92.8\% for all six sequences.

We must note that \citeauthor{weiss87} did not maintain the amino acid structure
while substituting codons.  Changes in the primary structure may be of interest in
...
Once again, it must be noted that like Weiss, \citeauthor{schober:bgh}
...
Results should be available by mid-October.  We are indebted to a graduate
student for running this experiment in the laboratory.
...
A recurring theme throughout the results is that outliers---certain
sequences by Weiss and pcZ108, for example---often differ in primary
structure from sequences known to translate at high rates.  This pattern
suggests a bottleneck in our model: It is designed to concentrated on
signal analysis of the mRNA, not on the biochemical interactions of
the nascent polypeptide with its ribosomal and cellular environment.
As such, we cannot distinguish between errors in translation due to
encoding in the signal and structure of the protein.  To circumvent this
issue, more data needs to be collected on constructs that substitute
synonymous codons to improve translational efficiency.

``More data" epitomizes the direction of future research.  The algorithm
used to calibrate tRNA values presented good values for bovine growth hormone,
but the program could not be run to its fullest extent.  Because of the small
data set, it was imperative to stay within a certain margin of error of
the values that \citet{lalit:embs} calculated.  However, with more biological
data, the algorithm can be run on numerous sequences, and we can deviate
from the given values with some more confidence.

The model certainly provides considerable predictive power over a broad spectrum
of biological topics, as exemplified by the investigations in this report.  Yet,
we must reconcile physical assumptions of the model with biological experimentation.
Choosing the wrong codon implicitly suggests that the ribosome always produces
long proteins, although some proteins may be dysfunctional.  Such is not always the case
[Dr. Stomp, we need some clarification or citations here].  The other interpretation
discussed (that deviation from the baseline entails a longer time for translation) may
bear more biological sense, as the ribosome is experimentally known to dissociate if translation takes
too long.  Thus, we must consider the biological implications of the model in addition
