...
\begin{abstract}\begin{normalsize}
  In ``\BWFtitle{\emph}'', we propose a stochastic model
...
the deterministic model, the displacement jumps to $x=2$.  Certain genes 
demonstrate ambiguous behavior near $x = \pm 1$~\cite{lalit:mechanics}. 
In the deterministic model, we lack the sensitivity needed to clearly 
discern a stable frameshift or detect unstable behaviors. Due to the 
noise within a cell, the nondeterministic behavior of translation limits 
the model's ability to predict displacement. Therefore, we designed a
stochastic model incorporating a sinusoidal probability paradigm.

We propose that at each cycle in elongation, the ribosome makes a
...
of staying in the that frame is one.  Then, a ribosome
...
At codon $abc$, let the probability of choosing it at a wait-cycle $k$
is $P_k$, which we know is inversely proportional to the weight $\omega
= \omega_{abc,k}$ where $n = n_{abc}$ is some constant of
proportionality. (Therefore, $P_k = \omega/n$.) Then the probability of
choosing codon $abc$ after many wait cycles (say, $K$ of them) is the
probability of not failing to choose them after that many, which is
the product of each failure at each wait-cycle $k$ because the event
of failure is independent. Therefore the probability of choosing codon
$abc$, i.e. the zero reading frame, is
\begin{equation}
  P = 1 - \prod^K \left(1 - \frac{\omega_{abc,k}}{n_{abc}}
  \right).\footnote{The sinusoidal component of $\omega_i$ is not
    always cosine and depends upon the frame choice in question.}
\end{equation}
The value of $n$ derives from the normalization of $N$ after assuming
the probability of choosing any codon is one-half after $N$ loops, $(1
- 1/n)^N = 1/2$.\footnote{
  This resolves to $n = \sqrt[N]{2}/(\sqrt[N]{2}-1)$.
} Also, importantly, $\omega$ depends upon
displacement, which increments after every cycle.
...
to its TAV through $N$, which coincides with intuition.
...
are not
crucial to the true interplay between free energy and
translational efficiency.

In addition, our model agrees with a great pool of biological
