...
In the past, the only practical method to determine whether or not a sequence often 
produces malformed protein or is translationally inefficient was to construct a 
reporter plasmid and run a series of experiments.  This process is not only 
time-consuming, but also expensive and requires specialized equipment.  Obviously, 
any method that could effectively shorten or cheapen this process would be extremely 
useful. We present such a method: a computational model.  Based on fundamental 
principles of physics, molecular biology, and statistics, as well as the groundwork 
of other researchers, we created a new stochastic model for ribosomal translation 
that incorporates probability, force, and free energy present in hybridization and 
which can fairly accurately predict programmed frameshifts, incorrect codon choice, 
and can also be incorporated into a wide variety of other research, such as 
optimization of tRNA availabilities for recombinant protein synthesis. 
...
\begin{cfigure}
  ...
  \subfloat[Displacement Plot]{
    \label{prfB:disp:sub}
    \includegraphics[width=0.5\textwidth]{prfB/disp}
  }
