...
it to an experimental error due to its similarity to pcZ114. In short,
they believe that the low protein yield for pcZ108 is not due to
translational effects, a hypothesis that possibly explains why our
model predicts a relatively high translation rate for pcZ108.  Excluding
this outlier from our data, we find that our data
(\autoref{bgh:deviation}) fully agrees with hers. In addition, and like
\citeauthor{weiss87}, \citeauthor{schoner:bgh} changed the sequence to encode a different
amino acid sequence than the other constructs. We can then possibly attribute
