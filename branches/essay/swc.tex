...
Weiss et al.~\cite{weiss87,weiss88} conducted research on the frameshift in \prfB, trying to determine factors that would affect translational rate.
...
\subsection{Determining tRNA Availability Values}

Ponnala et. al~\cite{lalit:embs} base the tRNA avaliability values on codon usage, surveying a set of 2438 verified genes from \ecoli.
Although this assumption has biological basis~\cite{ikemura}, it is still quite possible that the true values for these numbers may differ 
...
An initial population of size $k$ was created by randomly modifying one through four tRNA availibility values by a factor between 0.5 and 2 
and repeating this process $k$ times.
We calculate the mean $\mu_{\rm{low}}$ of the deviation yields of the six bGH sequences with the lowest biological yields
as well as the mean $\mu_{\rm{high}}$ of two sequences with the highest yields.
We sort the new tRNA availability values by $\mu_{\rm{high}}/\mu_{\rm{low}}$, a lower value being more optimal.
After discarding the worst half of the tRNA vectors, we choose two of the remaining vectors, 
with the probability of choosing each vector proportional to its rank.
We then take a weighted average of these two vectors to create a new tRNA vector.
We repeatedly choose and average tRNA vectors until a population of size $k$ is created for the next generation.
A set number of mutations are initiated per generation by randomly modifying one through four tRNA availability values for a few vectors.
The algorithm terminates after a fixed number of generations.

It is important to note, however, that changes in tRNA availability values were not drastic;
they did not deviate much from the biologically-based calculations of Ponnala et. al.~\cite{lalit:embs}.
